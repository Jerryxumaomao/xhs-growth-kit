...
1. 从 `starter-rubrics/<content_form>.md` 读默认 bucket 边界（除非用户在 rubric_notes.md 自定义了）。**优先用 rubric_notes.md 里的 bucket 段**——cold-start 形态（如 `xhs-zhongcao`、`xhs-zhangfen`）的 starter 是 `<content_form>-zero.md`，且 bucket 段已随 init 复制进 rubric_notes.md，正常不需要回读 starter 文件；只有 rubric_notes.md 缺 bucket 段时才回退，回退时按 `<content_form>.md` → 不存在则 `<content_form>-zero.md` 解析
