...
| 必填 | 来源 |
|---|---|
| `<prediction-file>` 或 `<video-folder>` | 用户参数；缺失则从 `.cheat-state.json` 的 `pending_retros` 中取 `retro_done` 为空、`published_at` 最早的一条的 `.file` |
...
- 缺省 → 从 `pending_retros` 取 `retro_done` 为空、`published_at` 最早的一条（legacy 字符串条目视为 `{file: <str>}`）
...
```json
{
  ...
  "pending_retros": [<默认剔除本次；如约定了后续追踪（T+7 长尾等）则保留本条并置 retro_done + note 写追踪计划>],
  "last_retro_at": "<ISO>",
...
- pending_retros：默认剔除本条；需长尾追踪则置 `retro_done` 保留（status/hook 不再把它算作待复盘）
