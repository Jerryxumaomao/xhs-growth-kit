...
```json
{
  ...
  "pending_retros": [
    {
      "file": "predictions/<filename>",
      "published_at": "<ISO timestamp，与 last_published_at 一致>",
      "video_folder": "videos/<...>/",
      "platform": "<xhs|douyin|...>",
      "retro_done": null,
      "note": ""
...
`pending_retros` 是复盘队列（object 形态，契约见 [state-management.md](../../shared-references/state-management.md) "列表队列"表）——`cheat-status` / SessionStart hook 基于 `retro_done` 为空的条目 + `published_at` + RETRO_WINDOW_DAYS 显示"今天该复盘哪些"。`published_at` 必填——hook 靠它算 T+N，不再依赖去 prediction 文件里 grep。
