...
- **自检 leak guard**：对 `rubric_notes.md` 跑 `grep -nE '\\d+(\\.\\d+)?\\s*[wWmMkK万]|播放|实绩'` 作为**候选发现器**，逐条命中做语义复核——允许模式（公式 / 维度定义 / bucket 边界如 `5-30w` / 抽象倍率）放行；违禁模式（具名样本标题+数字 / 实绩表述 / 评论摘录带赞数）→ **abort bump + 回滚**，内容移到 rubric-memo.md。复核结论写进升级 Memo。完整判则见 [observation-lifecycle.md](../../shared-references/observation-lifecycle.md) 的 leak guard 段
