...
argument-hint: [— form: opinion-video|xhs-zhongcao|xhs-zhangfen|long-essay|short-text|podcast]
...
> g) **混合**
> h) **小红书种草 / 好物分享**（银饰 / 香水 / 美妆 / 穿搭 的测评·盘点·教程，图文或视频）
>      — 有专用 rubric，北极星是**收藏率（藏曝比）**不是播放量；复盘配 xhs-explore 适配器
> i) **小红书涨粉 / 口碑**（现阶段重攒粉+口碑、不带货的真实分享，银饰/香水当人设载体）
>      — 有专用 rubric，北极星是**关注转化率**（口碑副信号=分享率+评论正向率），收藏降级；复盘配 xhs-explore 适配器"
...
- 选 a → `false`（直接匹配内置观点视频 rubric）
- 选 h → `false`（匹配小红书种草专用 rubric `xhs-zhongcao-zero.md`，北极星=收藏率）
- 选 i → `false`（匹配小红书涨粉/口碑专用 rubric `xhs-zhangfen-zero.md`，北极星=关注转化率）
- 选 b/c/d/e/f/g → `true`，cheat-status 持续提示"你的形态可能需要 bump 调权重"
- **不再有"严重不匹配"档**——所有形态都能跑工作流，只是有的 rubric 需要更激进的 bump

**Q1.5: 典型时长**（仅 Q1=a/d/f/h/i 时问；h/i=小红书若主要做图文可直接跳过，做视频笔记才问）
...
2. **`rubric_notes.md`**
   ...
   - content_form → starter 文件映射：`opinion-video`→`opinion-video-zero.md`；`xhs-zhongcao`→`xhs-zhongcao-zero.md`（小红书种草，北极星=收藏率，图文 /7 + 视频 /9 两套公式）；`xhs-zhangfen`→`xhs-zhangfen-zero.md`（小红书涨粉/口碑，北极星=关注转化率，图文 /7 + 视频 /9 两套公式）；其余形态用对应 `<form>-zero.md`，缺失则用 `other-zero.md` 起步并标 `rubric_form_mismatch`
   - **绝不覆盖或删除 starter-rubrics/ 里的其他 rubric**——各形态 rubric 互不影响（如选 h 不会动 opinion-video.md）
