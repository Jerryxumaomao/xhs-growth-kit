...
# pending_retros entries are objects {file, published_at, retro_done, ...}; legacy string entries = {file: <str>}.
# Only entries with empty/null retro_done count as pending (done entries kept for T+7 follow-ups don't).
pending_retros_count=$(echo "$state" | jq -r '[.pending_retros // [] | .[] | if type == "string" then {file: .} else . end | select((.retro_done // "") == "")] | length')
...
if [[ "$pending_retros_count" -gt 0 ]]; then
  # Walk pending entries (objects; legacy strings normalized to {file}).
  # published_at comes from the state entry itself; fall back to grepping the prediction file
  # (header may be bullet-style "- **Published at**: ..." — match loosely).
  while IFS=$'\t' read -r pred_file pub_iso; do
    [[ -z "$pred_file" ]] && continue
    if [[ -z "$pub_iso" ]]; then
  ...
  done < <(echo "$state" | jq -r '.pending_retros // [] | .[] | if type == "string" then {file: .} else . end | select((.retro_done // "") == "") | [.file, (.published_at // "")] | @tsv')
fi
