...
**强制 cheat-bump Phase 5 末尾自检**：写完 rubric_notes.md 后跑 `grep -nE '\\d+(\\.\\d+)?\\s*[wWmMkK万]|播放|实绩'` 作为**候选发现器**，然后逐条命中做语义复核：
- 命中属于**允许模式**（公式、维度定义、bucket 边界如 `5-30w`、抽象倍率如 `≈1.5× baseline`）→ 放行
- 命中属于**违禁模式**（具名样本标题+数字、实绩表述、评论摘录带赞数）→ **abort + 回滚**，内容移到 rubric-memo.md

不允许跳过复核直接放行，也不允许"命中即 abort"（会误杀协议明确允许的 bucket 边界）。复核结论（N 条命中 / M 条放行理由）写进 bump 的升级 Memo。
...
⚠️ **"落点"行只写 bucket / 偏差档 / 相对倍率**——具名播放数、点赞数、评论摘录原句写 `rubric-memo.md`，本文件（blind 白名单）不落。这就是下方 leak guard 的模板级执行。
...
### 观察 X — [一句话规律]
**证据**：
- 样本 1（composite, bucket 落点/偏差档）：核心信号（抽象化，无具名标题与数字）
- 样本 2（composite, bucket 落点/偏差档）：核心信号
- 样本 3（composite, bucket 落点/偏差档）：核心信号
