...
| `content_form` | enum | "opinion-video" / "xhs-zhongcao" / "xhs-zhangfen" / "long-essay" / "short-text" / "podcast" / "other" / "mixed" | cheat-init | cheat-predict / cheat-recommend |
...
| `rubric_form_mismatch` | bool | true 表示该形态没有自带匹配 rubric、借用别的 rubric 起步——提示用户 bump 时调权重。`opinion-video`、`xhs-zhongcao`、`xhs-zhangfen` 各有专用 rubric → false；其余形态 → true | cheat-init | cheat-status（持续提示） |
...
| `pending_retros` | array of object `{file, published_at, video_folder?, platform?, retro_done, note?}` | cheat-publish（push） / cheat-retro（标记或移除） | cheat-status / SessionStart hook | 复盘队列。`file`+`published_at` 必填（窗口判定依据）；`retro_done` 为 null/缺失 = 待复盘，非空（ISO 或 "T+3 (date)" 标签）= 已复盘。复盘完成默认移除本条；如需后续追踪（如 T+7 长尾）可保留：置 `retro_done` + 在 `note` 写追踪计划。读取方须兼容 legacy 字符串条目（视为 `{file: <str>}`） |
...
| `pending_retros` | cheat-publish（push）/ cheat-retro（标记或移除） | publish 时 push 本次（object 形态，见"列表队列"表）；retro 完成时默认移除，需长尾追踪则置 `retro_done` 保留 |
...
| `pending_retros` 含已删除的文件（条目 `.file` 指向不存在的路径） | cheat-status 检测时安静移除，不报错 |
| `pending_retros` 含 legacy 字符串条目 | 读取方按 `{file: <str>}` 兼容处理；cheat-status 检测时顺手升格为 object 形态 |
