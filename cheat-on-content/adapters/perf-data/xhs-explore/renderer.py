...
def render_report(note: dict, script: str, comments: list[dict]) -> str:
    ...
    if note.get("analyze_ok"):
        fans = note.get("fans_inc") or 0
        lines.append(f"- **涨粉：{fans}（关注转化率 {_ratio(fans, view)} ← 北极星）**")
        if note.get("imp_count"):
            lines.append(f"- 曝光(impression)：{_fmt_num(note.get('imp_count'))}"
                         f"（阅读 {_fmt_num(view)}，阅读率 {_ratio(view, note.get('imp_count'))}）")
        ccr = note.get("cover_click_rate")
        if ccr is not None:
            try:
                lines.append(f"- 封面点击率：{float(ccr) * 100:.1f}%（CA 封面维度的真实信号）")
            except (ValueError, TypeError):
                pass
    elif note.get("fans_inc"):
