...
# 数据看板→内容分析页：触发 note/analyze/list（每篇带 涨粉/曝光/封面点击率），按序试，命中即停
DATA_ANALYSIS_URLS = (
    "https://creator.xiaohongshu.com/statistics/data-analysis",
    "https://creator.xiaohongshu.com/new/data-analysis",
)
# 内容分析的逐篇接口——含 increase_fans_count（涨粉，posted 列表接口没有的关键字段）
GALAXY_NOTE_ANALYZE_KEY = "/creator/datacenter/note/analyze/list"
# 涨粉字段候选——analyze 接口与 posted 列表两处解析必须共用同一列表，防接口改版时一处漏更。
# 顺序即优先级：确认名在前；"fans" 在部分 payload 里可能是粉丝总量而非增量，放最后兜底。
FANS_INC_KEYS = ("increase_fans_count", "rise_fans_count", "fans_inc", "new_fans", "follow_count", "fans")
# 内容分析分页 replay 的兜底上限（10 页 ≈ 100 篇，个人号量级足够）
MAX_ANALYZE_PAGES = 10
# 分页参数候选名（GET query 或 POST JSON body）
_PAGE_PARAM_KEYS = ("page_num", "pageNum", "page", "page_no", "pageNo", "current_page", "current")
...
def _merge_analyze_blob(blob: Any, out: dict[str, dict]) -> int:
    """把一个 note/analyze/list response 合进 out，返回新增条数。"""
    added = 0
    infos = (((blob or {}).get("data") or {}).get("note_infos")) or []
    for n in infos:
        ...
        if not nid or nid in out:
            continue
        out[nid] = {
            "fans_inc": _to_int(_first(n, *FANS_INC_KEYS)),
            "imp_count": _to_int(_first(n, "imp_count", "impression", "imp")),
            "cover_click_rate": n.get("coverClickRate"),
            "read_count": _to_int(_first(n, "read_count", "view_count")),
            "analyze_ok": True,
        }
        added += 1
    return added
...
def _with_page(url: str, post_data: str | None, page_num: int) -> tuple[str, str | None] | None:
    """把 analyze 请求的分页参数改为 page_num，用于 replay 翻页。

    GET → 改 query；POST(JSON) → 改 body。识别不到分页参数返回 None（优雅降级只用第 1 页）。
    """
    ...
    for k in _PAGE_PARAM_KEYS:
        if k in q:
            q[k] = str(page_num)
            return urllib.parse.urlunsplit(parsed._replace(query=urllib.parse.urlencode(q))), post_data
    if post_data:
        try:
...
async def fetch_note_analyze_map(sess: Session, limit: int = 50) -> dict[str, dict]:
    """数据看板→内容分析页 → 拦截 note/analyze/list，拿每篇 涨粉 / 真实曝光 / 封面点击率。

    posted 列表接口**不含涨粉**；涨粉(increase_fans_count)只在「内容分析」接口里。
    返回 {note_id: {fans_inc, imp_count, cover_click_rate, read_count, analyze_ok}}。

    分页：navigate-intercept 只触发第 1 页（约 10 篇）；之后 replay 同一请求改分页参数翻
    第 2..MAX_ANALYZE_PAGES 页（共享 context cookie）。任一页失败/为空即收手——最坏情况
    退回旧行为（只有第 1 页），不会更差。
    """
    captured: list[dict] = []
    page = await sess.ctx.new_page()
    ...
    async def on_response(resp: Response) -> None:
        if GALAXY_NOTE_ANALYZE_KEY in resp.url:
            try:
    ...
    try:
        for url in DATA_ANALYSIS_URLS:
            ...
            if "login" in page.url:
                print("[诊断] 内容分析页跳登录，涨粉跳过（galaxy 主路不受影响）。")
                break
            for _ in range(3):
        ...
        for item in captured:
            _merge_analyze_blob(item["data"], out)

        if not out:
            print("[诊断] 未拦到 note/analyze/list 涨粉数据（接口或路由可能已变）。")
            return out

        # --- 分页 replay：用拦到的真实请求当模板，改 page 参数翻后续页 ---
        req0 = captured[-1]
        referer = {"referer": DATA_ANALYSIS_URLS[0]}
        pages_fetched = 1
        for page_num in range(2, MAX_ANALYZE_PAGES + 1):
            if len(out) >= limit:
                break
            mutated = _with_page(req0["url"], req0.get("post_data"), page_num)
            if mutated is None:
                print("[诊断] analyze 请求未识别到分页参数——本次只覆盖第 1 页（约 10 篇）。"
                      f" 请求样例已知：{req0['method']} {req0['url'][:120]}")
                break
            new_url, new_body = mutated
            blob = None
            try:
                if req0["method"] == "POST":
                    r = await sess.ctx.request.post(
                        new_url, data=new_body,
                        headers={**referer, "content-type": "application/json"})
                else:
                    r = await sess.ctx.request.get(new_url, headers=referer)
                if r.ok:
                    blob = await r.json()
            except Exception:
                blob = None
            added = _merge_analyze_blob(blob, out) if blob else 0
            if added == 0:
                break  # 翻到底 / 接口拒绝——优雅收手，已有页数据保留
            pages_fetched += 1
            await asyncio.sleep(1.0)  # 温和限速，别打riskcontrol
        print(f"[分页] 内容分析覆盖 {len(out)} 篇（{pages_fetched} 页）。")
    finally:
        await page.close()
    return out
...
def _normalize_note(v: dict) -> dict:
    ...
    return {
        ...
        "fans_inc": _to_int(_first(v, *FANS_INC_KEYS)),
...
async def fetch_all(note_id: str, note_url: str | None = None) -> dict:
    ...
    try:
        ...
        print("  → 打开内容分析页，拉每篇 涨粉 / 封面点击率")
        analyze = await fetch_note_analyze_map(sess)
        ...
        # 合并「内容分析」的涨粉 / 真实曝光 / 封面点击率（posted 列表没有这些）
        a = analyze.get(note_id)
        if a:
            note["fans_inc"] = a["fans_inc"]
            note["imp_count"] = a.get("imp_count") or 0
            note["cover_click_rate"] = a.get("cover_click_rate")
            note["analyze_ok"] = True
            print(f"       ✓ 涨粉 {a['fans_inc']} · 封面点击率 "
                  f"{(a.get('cover_click_rate') or 0) * 100:.1f}%")
