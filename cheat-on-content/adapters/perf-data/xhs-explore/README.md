...
| 涨粉显示 0 但后台有数 | analyze 接口涨粉字段改名 | 把真实 key 加进 `FANS_INC_KEYS`（analyze 与 posted 两处解析共用这一个常量） |
| 涨粉只覆盖最近 ~10 篇 | 内容分析分页 replay 未识别分页参数（接口改版） | 看控制台 `[诊断]` 打出的请求样例，更新 `_PAGE_PARAM_KEYS`；未修复前自动降级为只用第 1 页 |
